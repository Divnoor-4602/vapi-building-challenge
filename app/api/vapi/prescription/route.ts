...
import { routePrescriptionFunction } from "./router";

// Handle CORS preflight requests
export const OPTIONS = createCorsHandler();

export async function POST(req: NextRequest) {
  const logPrefix = "[PRESCRIPTION_ROUTE]";

  console.log(`${logPrefix} 🚀 Received prescription request`);
  ...
  try {
    ...
    const result = await routePrescriptionFunction(
      functionName,
      args,
      toolCallId
    );
